...
class TestGate(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.guard = cls.env['res.users'].create({
            ...
            'group_ids': [(6, 0, [cls.env.ref('base.group_user').id, cls.env.ref('gate_management.group_gate_guard').id])],
        })
        cls.manager = cls.env['res.users'].create({
            'name': 'Gate Manager', 'login': 'gate_manager', 'email': 'manager@example.com',
            'group_ids': [(6, 0, [cls.env.ref('base.group_user').id, cls.env.ref('gate_management.group_gate_manager').id])],
        })
    ...
    def test_portal_user_cannot_record_attendance(self):
        portal = self.env['res.users'].create({'name': 'Portal', 'login': 'portal_x', 'group_ids': [(6, 0, [self.env.ref('base.group_portal').id])]})
        with self.assertRaises(AccessError):
